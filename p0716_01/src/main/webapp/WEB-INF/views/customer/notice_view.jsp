					...
					<script>
						$(function(){							
							...
							$.ajax({ //자바와 제이쿼리를 연결해줌
								 ...
								 dataType:"json",
								 success:function(data){ //data 에 1이라는 값이 들어옴.
									alert("댓글이 저장되었습니다.");
									let htmlData = "";
							...
							$(document).on("click",".dBtn", function(){
								console.log("ul cno : "+ $(this).closest('ul').attr('id'));
							...
							//click - 불러왔을때 존재할 때만 가능(기존파일만 가능) : 정적
							//on - 취소: 불러온 페이지 중간 : 동적
							//document : 전체 문서 내에서
							//취소 
							...
							//수정완료버튼클릭 -> controller로 보냄
								...
								$.ajax({ //자바와 제이쿼리를 연결해줌
									 ...
									 success:function(data){ //data 에 1이라는 값이 들어옴.
										...
										//현재위치에 수정코드 입력
										$("#"+cno).html(htmlData);//html 없애고 넣는 거 prepend위쪽에 append 아래쪽에
									 },
									 error:function(){
										 alert("실패");
									 }
